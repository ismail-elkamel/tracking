...
When several OBJ files are uploaded, they are treated as one complete 3D scene. This is useful for kidney + cortex + artery + vein + tumor exports: the app keeps the relative 3D positions from the files, places the full group with one placement control, tracks anchors on any visible part, and redraws every uploaded OBJ in the final video. Vertex colors embedded in the OBJ are kept. Material colors from uploaded `.mtl` files are also kept. If no vertex/material colors are present, the app uses stable fallback colors from the file/material names, for example artery/aorta red, vein blue, tumor pink, cortex orange, and kidney cyan.

After uploading, use the `3D model placement` controls to move, scale, and rotate the complete model. With `Move/zoom 3D model with mouse` enabled, the projected model itself appears next to the point/region annotation canvas: drag the model to translate it and resize it to zoom. Rotations are in the same placement panel as sliders, so mouse movement does not accidentally redefine the 3D orientation. The combined preview is shown below those controls with the same canvas resizing. The app tracks projected OBJ anchors, estimates a global transform from them, and redraws the complete OBJ geometry in the final video. Red anchor circles are hidden by default. Regular tracking points stay visible on top, and `Compare models` still works.
