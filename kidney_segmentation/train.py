...
from kidney_segmentation.dataset import (
    KIDNEY_LABELS,
    KidneySegmentationDataset,
    discover_samples,
    filter_samples_with_kidney,
    split_samples,
)
from kidney_segmentation.models import build_model
from kidney_segmentation.models import DEFAULT_MODEL
from kidney_segmentation.export_onnx import export_checkpoint_to_onnx
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a binary kidney segmentation model.")
    parser.add_argument("--data-root", default="data/Instrument segmentation")
    parser.add_argument("--output-dir", default="kidney_segmentation/runs/kidney_model")
    ...
    parser.add_argument("--visdom-env", default="kidney_segmentation")
...
def main() -> None:
    ...
    if not args.keep_empty_samples:
        samples = filter_samples_with_kidney(samples)
        removed_count = discovered_count - len(samples)
        print(f"Removed samples without Kidney annotations: {removed_count}")
    if not samples:
        raise RuntimeError(
            "No samples with Kidney annotations found. "
    ...
    print("Positive kidney mask labels:")
    for label in sorted(KIDNEY_LABELS):
        print(f"  - {label}")
    if args.keep_empty_samples:
        print("Samples without Kidney annotations are kept as empty masks.")
    else:
        print("Samples without Kidney annotations are skipped.")

    train_ds = KidneySegmentationDataset(train_samples, image_size=args.image_size, augment=True)
    val_ds = KidneySegmentationDataset(val_samples, image_size=args.image_size, augment=False)
    test_ds = KidneySegmentationDataset(test_samples, image_size=args.image_size, augment=False)
